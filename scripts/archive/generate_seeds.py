...
def to_pg_array(lst):
    ...
    return escape_sql(f"{{{joined}}}")

# Helper to format JSONB literal for SQL
def to_jsonb(dct):
    js_str = json.dumps(dct, ensure_ascii=False)
    return escape_sql(js_str)
...
EXERCISES_DATA = {
    ...
    "push_standard": {
        ...
        "erros": [
            ...
            "Encolher os ombros em direção às orelhas (falta de depressão escapular)."
        ],
    ...
    "row_table_inverted": {
        ...
        "erros": [
            ...
            "Descer soltando o peso sem controle na fase excêntrica do movimento."
        ],
    ...
    "sissy_squat_assisted": {
        ...
        "dicas": [
            ...
            "Use o apoio da mão apenas para estabilizar o equilíbrio no plano de descida."
        ],
        "erros": [
    ...
    "glute_bridge": {
        ...
        "erros": [
            "Hiperextender a coluna lombar (criar arco nas costas) no topo em vez de estender o quadril.",
    ...
    "hip_thrust_unilateral": {
        ...
        "articulacoes": ["lombar", "quadril"],
        "estresse": {"lombar": "baixo", "quadril": "baixo"},
        "descricao": "Apoie a porção média das costas na cadeira e eleve uma perna do solo. Com o pé da perna ativa bem apoiado, desça o quadril lentamente. Empurre com o calcanhar ativo elevando o quadril com força e controle até alinhar a pelve com os ombros de forma horizontal. Pause e desça controladamente.",
        "dicas": [
...
# Paths to the seed SQL file
target_file_path = r"c:\Users\user\Documents\Projetos Antigravity\Hypotros\hypertropos\supabase\migrations\20260520000002_seed_exercicios.sql"
...
blocks = sql_content.split("INSERT INTO exercicios (")
header = blocks[0]
exercise_blocks = blocks[1:]
...
for block in exercise_blocks:
    ...
    id_line = lines[values_idx + 1]
    id_match = re.search(r"'\s*([^']+)\s*'", id_line)
    if not id_match:
        processed_blocks.append(block)
        continue
    ...
    if ex_id not in EXERCISES_DATA:
        print(f"Warning: Biomechanics data not found for exercise ID '{ex_id}'. Cleaning default placeholders only.")
        # Just clean default placeholders (e.g. remove [REVISAR: ])
        block_clean = re.sub(r"'\[REVISAR:\s*([^\]]+)\]'", r"'\1'", block)
        processed_blocks.append(block_clean)
        continue
        
    ex_data = EXERCISES_DATA[ex_id]
    
    # We will replace values at specific lines relative to values_idx
    # 1. Clean alternative name on line values_idx + 1
    # Replace like '[REVISAR: Wall Push-up]' with 'Wall Push-up'
    lines[values_idx + 1] = re.sub(r"'\[REVISAR:\s*([^\]]+)\]'", r"'\1'", lines[values_idx + 1])
    
    # 2. Update GRF, articulations, and joint stress on line values_idx + 5
    # Format of pg array: '{"ombro", "cotovelo"}'
    # Format of jsonb: '{"ombro": "baixo"}'::jsonb
    grf_val = ex_data["grf_percentual"] if ex_data["grf_percentual"] is not None else "NULL"
    art_val = to_pg_array(ex_data["articulacoes"])
    stress_val = to_jsonb(ex_data["estresse"])
    lines[values_idx + 5] = f"    {grf_val}, '{art_val}', '{stress_val}'::jsonb,"
    
    # 3. Update description, tips, and errors on line values_idx + 6
    desc_val = escape_sql(ex_data["descricao"])
    tips_val = to_pg_array(ex_data["dicas"])
    err_val = to_pg_array(ex_data["erros"])
    lines[values_idx + 6] = f"    '{desc_val}', '{tips_val}', '{err_val}',"
    
    # 4. Update scientific phrase on line values_idx + 7
    phrase_val = escape_sql(ex_data["frase_cientifica"])
    # We preserve the empty media url '' at values_idx + 7
    lines[values_idx + 7] = f"    '', '{phrase_val}',"
    
    # 5. Update contraindications on line values_idx + 9
    contra_val = to_pg_array(ex_data["contraindicacoes"])
    lines[values_idx + 9] = f"    '{contra_val}'"
...
output_content = header + "INSERT INTO exercicios (".join(processed_blocks)

# Check for any remaining "[REVISAR:" in the output file to make sure it's 100% clean
remaining_placeholders = re.findall(r"\[REVISAR:[^\]]*\]", output_content)
if remaining_placeholders:
    print(f"Error! There are still {len(remaining_placeholders)} placeholders left in the generated file:")
    for ph in remaining_placeholders[:10]:
        print(f"  - {ph}")
else:
    print("Flawless! Zero placeholders remaining.")

# Save back to the migration file
with open(target_file_path, "w", encoding="utf-8") as f:
    f.write(output_content)

print(f"Migration file successfully written at {target_file_path}")
